...
class FocalLoss(nn.Module):
    ...
    def forward(self, classifications, regressions, anchors, annotations, **kwargs):
        ...
        for j in range(batch_size):
            ...
            # Clamp classification predictions to avoid log(0) issues
            ...
            if torch.cuda.is_available():
                alpha_factor = alpha_factor.cuda()
            # Apply alpha factor and focal weight
            ...
            if positive_indices.sum() > 0:
                ...
                targets = torch.stack((targets_dy, targets_dx, targets_dw, targets_dh))
                targets = targets.t()
                ...
                if torch.cuda.is_available():
                    ...
                    regression_losses.append(torch.tensor(0).to(dtype))
        # Post-process and display results if images are provided
        ...
        if imgs is not None:
            ...
            imgs = ((imgs * [0.14914498, 0.24979779, 0.24437005] + [0.07607268, 0.15061471, 0.14015888]) * 255).astype(np.uint8)
